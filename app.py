...
def seedance_compose(sid, num):
    ...
    sysprompt = (
        ...
        "ЛИМИТ SHOT-BIT'ОВ НА ЧАНК — КРИТИЧНО (баг «герои не двигаются, лип-синк плавает между ними»):\n"
        "Seedance рендерит видео-чанк 5-12 секунд. ФИЗИЧЕСКИ невозможно вместить 5+ shot-reverse-shot склеек "
        "в один такой клип — модель просто игнорирует часть инструкций и выдаёт «один плоский two-shot + "
        "ленивый lipsync чередующихся персов». Это приводит к багу когда героиня вроде смотрит на собеседника, "
        "но при его реплике её голова дрейфует, а сама камера не двигается.\n"
        "Правила:\n"
        "  1. МАКСИМУМ 2 shot-bit'а на чанк (по умолчанию). Один shot на каждого спикера, не больше.\n"
        "  2. Если в чанке РЕПЛИКИ ОДНОГО спикера (монолог, серия его строк подряд) → 1 shot-bit на весь "
        "     чанк. Просто medium close-up на спикера, без склеек. Если внутри длинного монолога нужен "
        "     визуальный акцент — можно лёгкий «medium → tight close-up на ключевую фразу», это всё ещё "
        "     1 «склейка», 2 shot-bit'а.\n"
        "  3. Если в чанке ДВА спикера обменялись 2-3 репликами → 2 shot-bit'а: первый shot держит спикера A "
        "     на ВСЕХ его репликах подряд, второй shot склеивается на спикера B на ВСЕХ его репликах. НЕ "
        "     чередуй cuts на каждую реплику.\n"
        "     Пример НЕ ПИШИ: «Clara говорит → Cut Adrian говорит → Cut Clara говорит → Cut Adrian говорит».\n"
        "     Пиши ТАК: «OTS на Clara: она произносит обе свои реплики. Reverse на Adrian: он отвечает "
        "     обеими своими репликами».\n"
        "  4. Если в чанке 3+ обмена репликами между двумя — это знак что чанкер дал слишком плотный кусок. "
        "     Всё равно ограничься 2 shot-bit'ами: один спикер A целиком, второй спикер B целиком. "
        "     Реплики не теряются — они звучат, просто не каждая получает свою склейку.\n"
        "  5. ИСКЛЮЧЕНИЕ — экшен/физическая сцена (драка, погоня, неожиданное действие): можно 3 shot-bit'а "
        "     если это коротко и каждый shot держит одно событие («cut на нож в руке → cut на лицо жертвы → "
        "     cut на широкий план»). Не больше 3.\n"
        "  6. САМОПРОВЕРКА перед выводом: посчитай словосочетания «Cut to / Камера переходит / Склейка / "
        "     reverse shot / medium close-up X». Если их >2 в одном чанке — это перебор, объедини реплики "
        "     одного спикера в один shot.\n\n"
